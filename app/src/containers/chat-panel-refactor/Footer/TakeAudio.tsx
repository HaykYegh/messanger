...
import {ESC_KEY_CODE, MEDIA_ACCESS_TYPE} from "configs/constants";
...
export default class TakeAudio extends React.Component<ITakeAudioProps, ITakeAudioState> {
    ...
    handleEscPress = (event) => {
        if (event.keyCode === ESC_KEY_CODE && !this.props.isAudioRecording) {
    ...
    handleRecordingStart = (event: React.MouseEvent<HTMLDivElement>): void => {
    ...
    render(): JSX.Element {
        ...
        const {mediaToggle, setAudioContainerRef, isAudioRecording} = this.props;
        const localization: any = components().takeAudio;

        return (
                <div className="audio_record" id="audio_record" ref={setAudioContainerRef}>
                    {error ?
