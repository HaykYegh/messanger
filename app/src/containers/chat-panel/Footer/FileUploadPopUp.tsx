...
import {
    ...
    MOUSE_WHEEL_SPEED, MUSIC_EXT_LIST,
...
interface IFileSenderProps {
    sendFiles: (file: any, voiceDuration: number, caption?: string, thumb?: string, blob?: Blob, amplitudes?: string, m_options?: any) => void,
...
export default class FileUploadPopUp extends React.Component<IFileSenderProps, IFileSenderState> {
    ...
    componentDidMount() {
        Log.i("componentDidMount -> electron = ", remote.app)
        // const appInstance = (electron.app || electron.app);
        const appInstance = remote.app;
    ...
    handleKeydown = (event: any) => {
        ...
        const {loadedFiles, isLoadingFinished} = this.state;
        ...
        if (event.keyCode === ENTER_KEY_CODE && loadedFiles && loadedFiles.length && isLoadingFinished) {
            event.preventDefault();
            this.handleFileSend(event);
        }
    };
    // Component will unmount when FileUploadPopUp is closed and specified time is irrelevant
    _handleKeydown = throttle(this.handleKeydown, 999999999, {trailing: false});
    ...
    handleFileSend = ({}: React.MouseEvent<HTMLButtonElement>) => {
        ...
        for (const item of loadedFiles) {
            const {file, caption, thumbnail, blob, dimensions} = item;
            const sanitizedCaption: string = caption !== "" ? caption.trim() : caption;
            let thumb: any
            let m_option: any
            ...
            if (typeof thumbnail === "object" && thumbnail.url) {
                thumb = thumbnail.url;
                m_option = {
                    width: thumbnail.saveWidth,
                    height: thumbnail.saveHeight
                }
            } else {
                thumb = thumbnail
                m_option = null
            ...
            sendFiles(file, null, sanitizedCaption, thumb, blob, undefined, m_option);
        }
    ...
    handleCaptionChange = ({currentTarget}: React.KeyboardEvent<HTMLDivElement>): void => {
        ...
        const newState: IFileSenderState = {...this.state};
        const value: string = currentTarget.innerText;
    ...
    get currentFileBlob(): string {
        ...
        const {currentFile} = this.state;
        let thumb: any
        if (typeof currentFile.thumbnail === "object" && currentFile.thumbnail.url) {
            thumb = currentFile.thumbnail.url
        } else {
            thumb = currentFile.thumbnail
        }
        return currentFile && thumb ? type.concat(thumb) : currentFile && currentFile.file && getBlobUri(currentFile.file);
    }
    ...
    render(): JSX.Element {
        ...
        const isMusicFile: boolean = MUSIC_EXT_LIST.includes(file.type);
        const imageSrc: string = getFileIcon(file.type);
        let imageText: string = "";
        if (imageSrc === file_unknown) {
            if (file.type && file.type.length <= 3) {
                imageText = file.type && file.type.toUpperCase();
            } else {
                imageText = "FILE"
            }
        }
                    ...
                    {isLoadingFinished && <div className="file-upload-popup-container">
                            ...
                            <div className="list-block">
                                <div className="file-list" ref={ref => this.fileList = ref}>
                                    {loadedFiles.length !== 0 ? loadedFiles.map((loadedFile, i) => {
                                        ...
                                        const type = "data:image/jpeg;base64,";
                                        let thumb: any;
                                        if (typeof loadedFile.thumbnail === "object" && loadedFile.thumbnail.url) {
                                            thumb = loadedFile.thumbnail.url
                                        } else {
                                            thumb = loadedFile.thumbnail
                                        }
                                        const fileBlob: string = thumb ? type.concat(thumb) : getBlobUri(loadedFile.file);
                                        const hasThumbnail: boolean = thumb && (loadedFile.file.type.includes(VIDEO_MIME_TYPE) || loadedFile.file.type.includes(IMAGE_MIME_TYPE));
                                        const bigSizeImage: boolean = loadedFile.file.size / 1024 / 1024 > FILE_PREVIEW_MAX_SIZE;


                                        const isMusicFile: boolean = MUSIC_EXT_LIST.includes(fileInfo.type);
                                        const imageSrc: string = getFileIcon(fileInfo.type);
                                        let imageText: string = "";
                                        if (imageSrc === file_unknown) {
                                            if (fileInfo.type.length <= 3) {
                                                imageText = fileInfo.type.toUpperCase();
                                            } else {
                                                imageText = "FILE"
                                            }
                                        }
                        ...
                        <div className="popup-footer">
                            <div className="footer-content">
                                ...
                                <button className="cancel-btn" onClick={closeFileUploadPopUp}>
                                    {localization.cancel}
                                </button>
                                <button
                                    className={"send-btn"}
