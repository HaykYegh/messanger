...
interface IConversationCreateGroupProps {
    close: () => void;
    containerState: {
        isOpened: boolean
    };
...
interface IConversationCreateGroupState {
    loading: {
        init: boolean,
...
export default class ConversationCreateGroup extends React.Component<IConversationCreateGroupProps, IConversationCreateGroupState> {
    ...
    componentDidMount() {
        ...
        if (this.isMounted) {
            ...
            (async () => {
                ...
                const newState: IConversationCreateGroupState = {...this.state};
    ...
    render() {
        ...
        const {user, contactInfo, changeConversationType, conversationType, rendering, withoutConversation, autoFocus, forGroup} = this.props;
        const selectedContactIds: string[] = isNextStep ? [...Object.keys(selectedContacts)] : [];
        const premium = isMap(user) ? user.get("premium") : user.premium
        const C_Count = premium === "true" ? CONFERENCE_CALL_MEMBERS_COUNT_PREMIUM : CONFERENCE_CALL_MEMBERS_COUNT
        return (
            <>
...

                {
                    ...
                    <div
                        ...
                        className={`popup-container container-absolute ${conversationType === CONVERSATION_TYPES.group ? "container-group" : ""}`}>

                        {!isNextStep && <div className={"popup-header"}>
                            <div className="text-block">
                                <h2>{localization.selectContacts}<span> {selectedContactsCount} / {forGroup ? CONFERENCE_CALL_MEMBERS_COUNT_PREMIUM : C_Count} </span></h2>
                                <p className={`error-text ${((selectedContactsCount >= C_Count) || forGroup) ? "display" : "display_none"}`}/>
                            </div>
                        ...
                        <div className={"popup-footer"}>
                            <div className="footer-content">
                                {!withoutConversation && !isNextStep &&<button onClick={() => changeConversationType(CONVERSATION_TYPES.conversation)}
                                         className="btn-cancel">{localization.back}</button>}

                                {isNextStep &&
                                <button onClick={this.handleGoBackClick}
                                        className="btn-cancel">{localization.back}</button>}
                                ...
                                {!isNextStep &&
                                <button onClick={this.handleNextStepClick}
                                        disabled={selectedContactsCount === 0}
                                        className="btn-next">{localization.next}</button>}
                                {isNextStep &&
                                <button onClick={this.handleGroupCreate} disabled={isGroupNameEmpty}
                                        className="btn-next">{localization.create}</button>}
                            </div>
                        </div>
                    </div>
                }
