...
interface ITakeVideoProps {
    send: (file: File, voiceDuration: number, caption: string, thumb: string) => void;
...
export default class TakeVideo extends React.Component<ITakeVideoProps, ITakeVideoState> {
    ...
    componentDidMount(): void {
        ...
        this.video.on("finishRecord", () => {
            ...
            const blob: Blob = this.video.recordedData.video;
    ...
    handleVideoSend = async (): Promise<any> => {
        ...
        this.handleClosePopUp();
        const thumb = await getThumbnail(blob, true);
        send(files[0],  undefined, undefined, thumb); // Todo Move to saga
    };
