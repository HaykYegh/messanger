...
import "scss/pages/chat-panel/helpers/VoiceMessagePopUp"; // Todo remove it after fully complete style-comp.
import {
    AudioBlock,
    AudioDuration,
    AudioProgressBar,
...
export default class AudioMessage extends React.Component<IAudioMessageProps, IAudioMessageState> {
    ...
    handleMovePosition = (durationAtIndex: number): void => {
        ...
        if (amplitudes && amplitudes.length > 0) {
            ...
            for (let k: number = 0; k < len; k++) {
                ...
                voiceAmplitudeView.push(
                    <VoiceAmplitude
                        key={k}
                        onClick={() => this.handleMovePosition(durationOnIndex)}
                        style={{
                            width: "2px",
    ...
    render(): JSX.Element {
        ...
        const {playing, audioTime, audioFile, meta, showLoader, progress, loadCancelled, uploadStarted} = this.state;
        const file = message.get('file');
        const audio = file || audioFile;
        const infoDuration = audio && message.get("info");
        const duration = meta.duration && meta.duration > 0 ? Math.round(meta.duration) : (infoDuration ? Math.round(infoDuration) : 0.01);
              ...
              className="audio-block">
                {
                    showLoader ?
                        ...
                        <span
                            className={this.props.language === 'ar' ? `audio-block-icon${playing ? " pause-btn" : " play-btn"} audio-block-icon-ar` : `audio-block-icon${playing ? " pause-btn" : " play-btn"}`}
                            onClick={playing ? this.handleAudioPause : isStream ? this.handleAudioPlay : null}
                        />
                }
                <AudioProgressBar>
                    <ProgressBar language={this.props.language} >
                        {this.amplitudes}
                    </ProgressBar>
                    ...
                    {
                        duration &&
                        <AudioDuration language={this.props.language}>{playing ? _audioTime : _duration}</AudioDuration>
