...
export const Blowfish: any = function(key: string, mode: string): void {
...
Blowfish.prototype = {
    ...
    encrypt(text: string, iv: string): void {
        if (this.mode === "ecb") {
            return this.encryptECB(text);
        }

        if (this.mode === "cbc") {
            return this.encryptCBC(text, iv);
        }

        throw new Error("Неизвестный режим шифрования.");
    },

    decrypt(text: string, iv: string): void {
        if (this.mode === "ecb") {
            return this.decryptECB(text);
        }

        if (this.mode === "cbc") {
            return this.decryptCBC(text, iv);
        }

        throw new Error("Неизвестный режим шифрования.");
    },

    encryptECB(text: string): string {
        text = this.utf8Decode(text);
        const blocks: number = Math.ceil(text.length / 8);

        let encryptedString: string = "";
        for (let i: number = 0; i < blocks; i++) {
            let block: string = text.substr(i * 8, 8);
            if (block.length < 8) {
                let count: number = 8 - block.length;
                while (0 < count--) {
                    block += "\0";
                }
            }
            ...
            let xL: string;
            let xR: string;
            let xLxR: string = this.split64by32(block);
            xL = xLxR[0];
            xR = xLxR[1];
    ...
    encryptCBC(text: string, iv: string): string {
        text = this.utf8Decode(text);
        const blocks: number = Math.ceil(text.length / 8);
    ...
    decryptECB(text: string): string {
        const blocks: number = Math.ceil(text.length / 8);

        let decryptedString: string = "";
        for (let i: number = 0; i < blocks; i++) {
            const block: string = text.substr(i * 8, 8);
    ...
    decryptCBC(text: string, iv: string): string {
    ...
    F(xL: number): number {
        const a: number = xL >>> 24;
        const b: number = xL << 8 >>> 24;
        const c: number = xL << 16 >>> 24;
        const d: number = xL << 24 >>> 24;

        let res: number = this.addMod32(this.sBox0[a], this.sBox1[b]);
        res = this.xor(res, this.sBox2[c]);
        res = this.addMod32(res, this.sBox3[d]);
        return res;
    },

    encipher(xL: string, xR: string): Array<string> {
        let tmp: string;
    ...
    decipher(xL: string, xR: string): Array<string> {
        let tmp: string;
    ...
    generateSubkeys(key: string): void {
        let data: number = 0;
        let k: number = 0;
        let i: number;
        let j: number;
    ...
    block32toNum(block32: string): number {
    ...
    fixNegative(num: number): number {
        return num >>> 0;
    },

    split64by32(block64: string): Array<string> {
        const xL: string = block64.substring(0, 4);
        const xR: string = block64.substring(4, 8);
    ...
    utf8Decode(text: string): string {
        ...
        for (let n: number = 0; n < text.length; n++) {
            const c: number = text.charCodeAt(n);
    ...
    utf8Encode(utftext: string): string {
        let text: string = "";
        let i: number = 0;
        let c: number = 0;
        let c1: number = 0;
