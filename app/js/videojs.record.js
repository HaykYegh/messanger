...
}(this, function (videojs)
{
    ...
    // monkey-patch play for photo.js 6.0 and newer (#149)
    ...
    videojs.RecordBase = videojs.extend(VjsComponent,
        {
            ...
            addFileInfo: function(fileObj)
            {
                ...
                // mime types like photo/webm;codecs=h264,vp9,opus
    ...
    videojs.RecordRTCEngine = videojs.extend(videojs.RecordBase,
        {
             ...
             *     example: {'photo': 'name-of-photo-file'}. Supported keys are
             *     'audio', 'photo' and 'gif'.
             */
             ...
             * @param {string} type - Media type, eg. 'photo' or 'audio'.
             */
                ...
                this.engine.getBlob(function(recording)
                {
                    switch (recordType)
                    {
                        ...
                        case this.AUDIO_VIDEO:
                            // when recording both audio and photo, recordrtc
                            // calls this twice on chrome, first with audio data
                            // and then with photo data.
                            // on firefox it's called once but with a single
                            // blob that includes both audio and photo data.
                            if (recording.video !== undefined)
                            {
                                // data is photo-only but on firefox audio+photo
                                this.recordedData = recording.video;

                                // on the chrome browser two blobs are created
                                // containing the separate audio/photo streams.
                                if (recordType === this.AUDIO_VIDEO && this.isChrome())
                                {
                                    // store both audio and photo
    ...
    /**
     * Record audio/photo/images using the Video.js player.
     *
     * @class
     * @augments videojs.RecordBase
     */
    videojs.Recorder = videojs.extend(videojs.RecordBase,
        {
                ...
                if (!this.isProcessing())
                {
                    ...
                    switch (this.getRecordType())
                    {
                        ...
                        case this.ANIMATION:
                            ...
                            {
                                // start photo preview **after** capturing first frame
                    ...
                    switch (this.getRecordType())
                    {
                        ...
                        case this.AUDIO_VIDEO:
                        case this.ANIMATION:
                            // wait for media stream on photo element to actually load
                            var self = this;
                            this.player().one('loadedmetadata', function()
                            {
            ...
            onRecordComplete: function()
            {
                ...
                switch (this.getRecordType())
                {
                    case this.AUDIO_ONLY:
                        ...
                        // will trigger an async photo.js 'pause' event that we
                        // have to wait for.
                    ...
                    case this.AUDIO_VIDEO:
                        ...
                        // store recorded data (photo-only or firefox audio+photo)
                        this.player().recordedData = this.engine.recordedData;
                        ...
                        this.player().one('pause', function()
                        {
                            ...
                            // and photo in the Chrome browser, playback the audio
                            // stream in a new extra audio element and the photo
                            // stream in the regular photo.js player.
             ...
             * for Chrome in audio+photo mode is present.
             * @private
             */
            ...
            load: function(url)
            {
                switch (this.getRecordType())
                {
                    ...
                    case this.AUDIO_VIDEO:
                    case this.ANIMATION:
                        // assign stream to audio/photo element source
             ...
             *     For example: {'photo': 'name-of-photo-file'}. Supported keys are
             *     'audio', 'photo' and 'gif'.
             */
            saveAs: function(name)
            {
            ...
            onPlayerStart: function()
            {
                ...
                {
                    // sync extra audio playhead position with photo.js player
            ...
            {
                // pause extra audio when photo.js player pauses
                if (this.extraAudio !== undefined)
                {
    ...
    var defaults = {
        ...
        audio: false,
        // Include photo in the recorded clip.
        video: false,
        // Animated GIF.
        animation: false,
        // Maximum length of the recorded clip.
        maxLength: 10,
        // Width of the recorded photo frames.
        frameWidth: 320,
        // Height of the recorded photo frames.
        frameHeight: 240,
        // Enables console logging for debugging purposes.
        debug: false,
        // The mime type for the photo recorder. Default to 'photo/webm'.
        // Use 'photo/mp4' (Firefox) or 'photo/webm;codecs=H264' (Chrome 52 and
        // newer) for MP4.
