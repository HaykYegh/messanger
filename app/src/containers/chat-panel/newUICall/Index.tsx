...
import {
    ...
    showCallRedialScreen,
    hideCallRedialScreen,
    TOGGLE_IGNORE,
    toggleMic,
    endCall,
...
import {getName, writeLog} from "helpers/DataHelper";
...
interface ICallPanelProps extends IStoreProps, ICallPanelPassedProps {
    ...
    endCall: () => void;
    showCallRedialScreen: () => void;
    hideCallRedialScreen: () => void;
...
class CallPanel extends React.Component<ICallPanelProps, ICallPanelState> {
    ...
    handleCallStart = (isVideo: boolean, contact: IContact, outCall: boolean) => {
        Log.i("radial call -> handleCallStart", outCall)
    ...
    acceptCall = (isVideo: boolean) => {
        ...
        // const desc: RTCSessionDescription = new SessionDescription({type: "offer"});

        this.peerConnection.setRemoteDescription(desc).then(() => {
            Log.i("conference -> RTCSessionDescription desc = ", desc)
            ...
            .catch((e) => {
                ...
                Log.i("conference -> RTCSessionDescription desc = stream 1", desc)
                this.setState({myUrl: URL.createObjectURL(stream)});
                ...
                this.peerConnection.createAnswer()
                    .then((answer) => {
                        Log.i("conference -> RTCSessionDescription desc = stream 2", answer)
                        return this.peerConnection.setLocalDescription(answer);
                    })
                    .then(() => {
                        acceptCall(lastCall.get("id"), lastCall.get("to"), this.peerConnection.localDescription.sdp);
                        Log.i("conference -> RTCSessionDescription desc = stream 3", this.peerConnection.localDescription.sdp)
    ...
    componentDidMount(): void {
        const {callDetails, lastCall, setParentState, contacts, removeCall, showNotification, app} = this.props;
    ...
    shouldComponentUpdate(nextProps: ICallPanelProps, nextState: ICallPanelState): boolean {
        const {
            lastCall, languages, minimized, showChat, app, contacts, isRedialScreenShown, callDetails,
            isStartedCall,
        } = this.props;
        const {callerContact} = this.state;
        ...
        if (isStartedCall !== nextProps.isStartedCall) {
            return true;
        }
    ...
    componentDidUpdate(prevProps: ICallPanelProps, prevState: ICallPanelState): void {
        const {
            lastCall, callDetails, removeCallDetails, removeIceCandidate, contacts, showNotification,
            sendLocalDescription, isStartedCall, endCall,
        } = this.props;
        const {callerContact, radialCall} = this.state;

        if (prevProps.isStartedCall !== isStartedCall && isStartedCall === true && !radialCall) {
            this.handleCallStart(false, callerContact, false);
        }
        ...
        if (lastCall && !prevProps.lastCall && this.props.isRedialScreenShown) {
            const {hideCallRedialScreen, minimized, setParentState} = this.props;
            hideCallRedialScreen();
            endCall();
        ...
        if (lastCall && prevProps.lastCall) {
            ...
            if (lastCall.get("status") !== prevProps.lastCall.get("status") && [CALL_STATUSES.decline].includes(lastCall.get("status"))) {
                const {removeCall, showCallRedialScreen} = this.props;
        ...
        if (!lastCall && !callDetails && !this.props.isRedialScreenShown) {
            this.setState({
                callerContact: null,
            })
        }
    ...
    render(): JSX.Element {
        ...
        const {lastCall, minimized, threadIsEmpty, isRedialScreenShown, selectedThreadId} = this.props;
        const localization: any = components().callPanel;
        ...
        if (isRedialScreenShown) {
            ...
            const {hideCallRedialScreen} = this.props;

            const name: string = getName(callerContact);

            return (
                <CallContent
                    localization={localization}
                    handleCallRadial={this.handleCallRadial}
                    handleRadialScreenClose={hideCallRedialScreen}
        ...
        const {contacts, notification, calls} = this.props;
        ...
        const callerInformation = lastCall && lastCall.get("ownCall") ? receiver : caller;
        const name: string = getName(callerInformation);
        const firstName: string = callerInformation && callerInformation.get("firstName");
            ...
            lastCall && lastCall.get("ownCall") || this.isAnswering ? (
                    <React.Fragment>
                        <CallContent
                            ...
                            receiverRequest={lastCall.get("receiverRequest")}
...
const mapDispatchToProps: any = dispatch => ({
    ...
    showCallRedialScreen: () => dispatch(showCallRedialScreen()),
    hideCallRedialScreen: () => dispatch(hideCallRedialScreen()),
